...
length(unique(df$pair_id)) # 1047 pairs
...
another_list_named <- list(
  first_item = TRUE, 
  second_item = c(1, 2, 3), 
  third_item = c("four", "five", "six")
)
another_list_named
...
pdf("01_intro_to_r/results/all_plots_mtcars.pdf", width = 8, height = 6)
print(mtcars_nested$mpg_vs_wt_plot)
dev.off()
...
plot_multiple_vars_per_pair <- function(dataset){
  ...
  plot_speed <- ggplot(data = dataset) +
    ...
    labs(x = "Time (s)",
         y = "Speed (m/s)",
        title = paste("Pair ID:", unique(dataset$pair_id))) +
    theme_classic()
...
df_nested_tjs <- df |> 
  ...
  mutate(combined_plot = map(.x = data, .f = plot_multiple_vars_per_pair))



df_nested_tjs$combined_plot[[1]]
df_nested_tjs$combined_plot[[15]]

pdf("01_intro_to_r/results/traj_plots.pdf", width = 8, height = 11)
print(df_nested_tjs$combined_plot)
dev.off()
